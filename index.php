<?php
session_start();
include 'portraits/config.php'; // Make sure to include your database connection file
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Function to check customer login with hashed password
    function checkCustomer($pdo, $email_or_username, $password) {
        $query = "SELECT * FROM customers WHERE (email = :email_or_username OR username = :email_or_username)";
        $stmt = $pdo->prepare($query);
        $stmt->execute(['email_or_username' => $email_or_username]);
        $customer_result = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($customer_result && password_verify($password, $customer_result['password'])) {
    ...
    if ($admin_result) {
        ...
        header("Location: it28-admin/public/dashboard.php");
        exit();
    }
    
    $customer_result = checkCustomer($pdo, $email_or_username, $password);
    if ($customer_result) {
